...
podman-build: build
	podman build --tag go-short --build-arg TARGETARCH=amd64 -f deploy/Containerfile.alpine .

podman-stop:
	podman ps -q --filter "name=go-short" | xargs -r podman stop
	podman ps -aq --filter "name=go-short" | xargs -r podman rm
...
podman-run: podman-stop
	podman run -t -p ${CHHOTO_LISTEN_PORT}:${CHHOTO_LISTEN_PORT} --name go-short --env-file ./.env -v "${DB_DIR}:/data" -d go-short
	podman logs go-short -f 
